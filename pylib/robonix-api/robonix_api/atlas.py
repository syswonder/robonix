...
class _Atlas:
    ...
    def _ensure_stub(self) -> None:
        ...
        with self._stub_lock:
            ...
            self._endpoint = ep
            self._pb = atlas_pb2
            # Atlas is always a local / LAN control-plane endpoint, so the
            # channel must NEVER honor `http_proxy`/`https_proxy` from the
            # environment. If it does, gRPC tunnels the local atlas connection
            # through whatever those vars point at and the dial fails. Disable
            # proxy resolution for this channel so the endpoint is always
            # reached directly.
