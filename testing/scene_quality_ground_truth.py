...
def _own_fields(block: str) -> str:
    r"""Return only the lines that belong to this node, not to a nested one.

    A regex for `^\s*translation ...` matches inside child nodes too. When
    e5358942 added a cameraSlot to TiagoLite, the first `translation` in the
    block became the camera's -0.028 -0.035 -0.009 offset instead of the
    robot's 0.962 -4.801 world pose. Every ground-truth centre is expressed
    relative to the robot's initial pose, so that silently shifted the whole
    fixture by 4.9 m and every score computed against it was meaningless.

    Depth is evaluated before the line is consumed, and the node's own opening
    brace is skipped, so the fields written directly on the node stay at 0.
    """
    kept: list[str] = []
    depth = 0
    for line in block.splitlines():
        ...
        # The node's own opening brace puts its direct fields at depth 1, so
        # keep a line when it starts there and opens nothing itself.
        if depth == 1 and opened == 0:
            kept.append(line)
        depth += opened - closed
        if depth < 0:
            depth = 0
...
def _resolve_target(
    ...
    *,
    ...
    # Read from the robot's own fields. A cameraSlot child carries its own
    # translation, and picking that up puts the map origin metres away, which
    # silently corrupts every truth derived from it.
    robot_own = _own_fields(robot)
    robot_xyz = _vector_field(robot_own, "translation", 3)
    target_xyz = _vector_field(target, "translation", 3)
    robot_yaw = _planar_yaw(_vector_field(robot_own, "rotation", 4))
