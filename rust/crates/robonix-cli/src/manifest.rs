...
pub struct Manifest {
    ...
    /// (`RBNX_PACKAGE_ROOT`, `RBNX_BUILD_CLEAN=1` when `--clean`).
    #[serde(default)]
...
pub struct Node {
    ...
    /// Shell command for `rbnx start -n <id>` (run from package root via `bash -lc`, after env exports).
    #[serde(default)]
    pub start: String,
    /// Optional `module:main` used only by `rbnx ros2-workspace-build` to generate ament_python setuptools metadata.
...
/// Each node must declare a non-empty `start` shell command.
pub fn validate_node_specs(manifest: &Manifest) -> Result<()> {
...
fn build_config_present(manifest: &Manifest) -> Result<()> {
    if manifest.build.script.trim().is_empty() {
        anyhow::bail!("manifest.build.script is required (path to a build script under the package)");
    }
    Ok(())
}
