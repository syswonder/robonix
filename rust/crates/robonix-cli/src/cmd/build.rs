...
/// Touched after `rbnx build` runs the package `build.script` successfully.
...
/// RIDL → ridlc → colcon under `rbnx-build/ws`. Intended to be called from a package `build.script`
/// (e.g. `rbnx ros2-workspace-build -p "$RBNX_PACKAGE_ROOT"`), not wired directly to `rbnx build`.
pub fn build_ros2_workspace(package_root: &Path, clean: bool) -> Result<()> {
...
"#
    ...
    // ROS-oriented path: expand the repo RIDL catalog into a colcon workspace (Python stubs + vendor msgs).
    output::step("Generating", "RIDL workspace into rbnx-build/ws (ridlc)");
...
fn build_local(package_root: &Path, manifest: &Manifest, clean: bool) -> Result<()> {
...
pub async fn execute_ros2_workspace_build(package: PathBuf, clean: bool) -> Result<()> {
    let root = package
        .canonicalize()
        .with_context(|| format!("Failed to canonicalize {}", package.display()))?;
    output::action(
        "ROS2 workspace (ridlc + colcon)",
        &format!("{}", root.display()),
    );
    build_ros2_workspace(&root, clean)?;
    Ok(())
}
