...
/// Load the package manifest and start all declared nodes in background.
async fn start_package(
    ...
    output::step(
        "Starting",
        &format!("{} ({} node(s))", entry.name, m.nodes.len()),
    );
    ...
    for node in &m.nodes {
        ...
        let label = format!("{}:{}", entry.name, node.id);
        ...
        output::sub_step(&format!("  {} (PID {})", label, child.id().unwrap_or(0)));
        children.push((label, child));

        // Brief pause between nodes to allow initialization.
