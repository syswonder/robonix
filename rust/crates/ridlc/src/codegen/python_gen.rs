            ...
            if field.is_array {
                match field.array_size {
                    Some(n) => {
                        ...
                        out.push(FlatField { name: prefix.to_string(), ctypes_type: ct, comment: None });
                    }
                    None => {
                        ...
                        let (cap_expr, cap_note) = if p == "uint8" || p == "byte" {
                            ("_MAX_IMAGE_BYTES", format!("unbounded {}[]; capped at _MAX_IMAGE_BYTES", p))
                        } else {
                            ("_MAX_ARRAY_ELEMS", format!("unbounded {}[]; capped at _MAX_ARRAY_ELEMS", p))
                        };
        ...
        MsgTypeRef::Named { package, name } => {
            if field.is_array {
                ...
                out.push(FlatField {
                    ...
                    comment: Some(format!("opaque blob: {}/{}[] not flattenable", package, name)),
                });
                return;
            }
...
pub fn generate(resolver: &MsgResolver, out_dir: &Path) -> Result<()> {
    ...
    for package in &all_packages {
        ...
        let _ = writeln!(out, "# @generated by ridlc --lang python — DO NOT EDIT");
        let _ = writeln!(out, "# source: ROS IDL package '{}'", package);
        let _ = writeln!(
            out,
            "# Payload types for the iceoryx2 Python API (ctypes.Structure + type_name())"
        );
        let _ = writeln!(out, "# Re-generate: cargo run -p ridlc -- --lang python -I <lib> -o <out>");
        let _ = writeln!(out);
        let _ = writeln!(out, "import ctypes");
        let _ = writeln!(out);
        // Module-level max sizes for unbounded arrays — override before first use.
        let _ = writeln!(out, "# Max element counts for unbounded (variable-length) array fields.");
        let _ = writeln!(out, "# iceoryx2 requires fixed-size payloads; these caps are the upper bound.");
        let _ = writeln!(out, "# Override the constant in your code before importing these types if needed.");
        let _ = writeln!(out, "_MAX_IMAGE_BYTES = {}  # 1080p RGBA upper bound", UNBOUNDED_UINT8_MAX);
        let _ = writeln!(out, "_MAX_ARRAY_ELEMS = {}  # generic upper bound for non-byte arrays", UNBOUNDED_ELEM_MAX);
        let _ = writeln!(out);
        let _ = writeln!(out);
    ...
    if !init.exists() {
        fs::write(&init, "# @generated by ridlc --lang python\n")?;
    }
