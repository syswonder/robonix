...
pub fn generate(
    ...
    if paths.is_empty() {
        eprintln!("[ridlc] contracts: no .toml under {}", contracts_dir.display());
        return Ok(());
    }
    ...
    for p in paths {
        let raw = fs::read_to_string(&p)
            .with_context(|| format!("read contract {}", p.display()))?;
        let c: ContractToml = toml::from_str(&raw)
    ...
    writeln!(
        &mut out,
        "// @generated by ridlc (--contracts). Source: TOML under {}.",
    ...
    for ((_, c), (_, in_t, out_t)) in contracts.iter().zip(proto_types.iter()) {
        ...
        let rpc = match mode {
            ...
            other => bail!("unknown [mode].type '{other}' in contract {}", c.contract.id),
        };
        writeln!(&mut out, "  {rpc}")?;
        writeln!(&mut out, "}}")?;
        writeln!(&mut out)?;
    ...
    eprintln!(
        "[ridlc] contracts: wrote {} ({} services)",
        outfile.display(),
        contracts.len()
    );
    Ok(())
...
fn resolve_io(
    ...
    bail!("unsupported [io] reference '{s}' (expected pkg/msg/Name, pkg/srv/Name, protobuf://..., or primitive/string)")
}
...
fn proto_file_for_dot_package(pkg: &str) -> String {
    // robonix.pilot → pilot.proto ; robonix.executor → executor.proto (ridlc package files)
