...
// interface to flip it to RUNNABLE — that's where the skill actually
...
async fn ensure_skill_runnable(atlas: &mut AtlasClient, cap_id: &str) -> Result<()> {
    ...
    if rec.state == atlas_pb::CapabilityState::StateRunnable as i32 {
        log::info!("[skill-activate] {cap_id}: already RUNNABLE per atlas, marking sticky");
