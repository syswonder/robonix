...
pub struct QueryNodesOpts {
    ...
    pub container_id: String,
    /// When non-empty, server matches `namespace + "/" + DeclareInterface.name` exactly; `namespace` and `interface_name` fields are ignored.
    pub abstract_interface_id: String,
}

impl RobonixClient {
    pub async fn connect(endpoint: &str) -> Result<Self> {
        let inner = proto::robonix_runtime_client::RobonixRuntimeClient::connect(endpoint.to_owned())
            .await
            .context("failed to connect to robonix-server")?;
        Ok(Self { inner })
    }

    pub async fn connect_with_retry(endpoint: &str, attempts: usize, delay: Duration) -> Result<Self> {
        for i in 0..attempts {
    ...
    pub async fn register_node(
        ...
        self.register_node_full(node_id, namespace, kind, skill_md, "", "").await
    }
    ...
    pub async fn register_node_full(
        ...
        let resp = self.inner.register_node(proto::RegisterNodeRequest {
        ...
        }).await.context("register_node RPC failed")?;
    ...
    pub async fn declare_interface(
        ...
        self.declare_interface_with_listen_port(node_id, name, supported_transports, metadata_json, 0)
            .await
    }
    ...
    pub async fn negotiate_channel(
        ...
        let resp = self.inner.negotiate_channel(proto::NegotiateChannelRequest {
            consumer_id: consumer_id.into(),
            provider_node_id: provider_node_id.into(),
            interface_name: interface_name.into(),
            transport: transport.into(),
        }).await.context("negotiate_channel RPC failed")?;
        let r = resp.into_inner();
        Ok(Channel { channel_id: r.channel_id, transport: r.transport, endpoint: r.endpoint })
    }

    pub async fn release_channel(&mut self, channel_id: impl Into<String>) -> Result<()> {
        self.inner.release_channel(proto::ReleaseChannelRequest {
            channel_id: channel_id.into(),
        }).await.context("release_channel RPC failed")?;
        Ok(())
    }

    pub async fn query_skill_md(&mut self, node_id: impl Into<String>) -> Result<String> {
        let resp = self.inner.query_skill_md(proto::QuerySkillMdRequest {
            node_id: node_id.into(),
        }).await.context("query_skill_md RPC failed")?;
        Ok(resp.into_inner().skill_md)
    }

    pub async fn query_all_skills(&mut self) -> Result<Vec<SkillEntry>> {
        let resp = self.inner.query_all_skills(proto::QueryAllSkillsRequest {})
            .await.context("query_all_skills RPC failed")?;
        Ok(resp.into_inner().skills.into_iter().map(|s| SkillEntry {
            node_id: s.node_id,
    ...
    pub async fn inspect_runtime(&mut self) -> Result<String> {
        let resp = self.inner.inspect_runtime(proto::InspectRuntimeRequest {})
            .await.context("inspect_runtime RPC failed")?;
