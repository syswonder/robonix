...
class BenchmarkReport:
    def __init__(self, log_dir, output_file=None, concurrency=None):
        self.log_dir = Path(log_dir)
        self.output_file = Path(output_file) if output_file else (self.log_dir / "benchmark_report.json")
        self.results = {"stats": {}, "metadata": {"timestamp": datetime.now().isoformat(), "log_dir": str(self.log_dir), "concurrency": concurrency}}

    def parse_log(self, content):
        stats = {"total": 0, "success": 0, "failed": 0, "avg": 0, "min": 0, "max": 0, "p50": 0, "p95": 0, "p99": 0, "p999": 0}
        m = re.search(r'Overall Statistics:.*?Total: (\d+), Success: (\d+), Failed: (\d+).*?Latency \(ms\): Avg: ([\d.]+), Min: ([\d.]+), Max: ([\d.]+), P50: ([\d.]+), P95: ([\d.]+), P99: ([\d.]+), P999: ([\d.]+)', content, re.DOTALL)
        if m:
