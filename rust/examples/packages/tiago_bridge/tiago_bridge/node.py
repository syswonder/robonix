...
Optional env vars:
  ...
  ROBONIX_NAMESPACE    Registry namespace (default: robonix/prm/tiago)
...
def _start_ros2():
    ...
    global _ros_node, _goal_pub, _cmd_vel_pub, _nav_client, _nav_action_ready
    _import_ros2()
    _rclpy.init()
...
def main() -> None:
    ...
    stub.RegisterNode(
        pb.RegisterNodeRequest(
            node_id=node_id,
            namespace=os.environ.get("ROBONIX_NAMESPACE", "robonix/prm/tiago"),
            kind="primitive",
            skill_md="",
    ...
    print("[tiago-node] ready — ROS2 topics → MCP tools bridge active")
