...
"""Stream demo: pose subscriber. Continuously reads from stream_server (output stream).
...
def main() -> None:
    ...
    def on_msg(msg: PoseWithCovarianceStamped) -> None:
        received.append(msg)
        print(f"[{len(received)}] pose: x={msg.pose.pose.position.x:.2f} y={msg.pose.pose.position.y:.2f}", flush=True)
    ...
    if not received:
        ...
        sys.exit(1)
    print(f"Received {len(received)} messages (output stream test OK)", flush=True)
