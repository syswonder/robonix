...
#   - robonix-atlas must be running on ROBONIX_ATLAS (default localhost:50051)
...
#
# Override env vars:
#   ROBONIX_ATLAS=host:port  ENV_GRPC_ENDPOINT=host:port  RERUN_GRPC_URL=...
...
docker run --rm -i \
    --network host \
    --ipc host \
    -e ROBONIX_ATLAS="${ROBONIX_ATLAS:-localhost:50051}" \
