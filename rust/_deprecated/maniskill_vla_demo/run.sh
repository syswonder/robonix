...
_DEMO_PIDS=()
_DEMO_CLEANUP_DONE=0
bg() { "$@" & _DEMO_PIDS+=($!); }

cleanup() {
  # INT then EXIT both fire on Ctrl+C — only run once. Avoids duplicate kills / odd trap nesting.
  [ "$_DEMO_CLEANUP_DONE" = "1" ] && return 0
  _DEMO_CLEANUP_DONE=1
  trap - INT TERM EXIT
  ...
  if [ -f "$EXAMPLES_ROOT/stop.sh" ]; then
    bash "$EXAMPLES_ROOT/stop.sh" || true
  fi
...
PY
  ...
  # perception_node (disabled in manifest for Phase 1; uncomment there + here to re-enable)
  if [ "${START_PERCEPTION:-0}" = "1" ]; then
  ...
  # grasp_node (disabled in manifest for Phase 1)
  if [ "${START_GRASP:-0}" = "1" ]; then
  ...
  # vla_node (OpenVLA / scripted policy)
  if [ "${START_VLA:-1}" = "1" ]; then
    echo "[demo] starting vla_node..."
    bg bash -c "cd '$RUST_ROOT' && exec cargo run -p robonix-cli -- \
  ...
  # Requires: docker, env_node running, proto_gen/ generated by build step.
