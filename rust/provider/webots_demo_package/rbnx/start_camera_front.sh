...
# Start Front Camera Primitive Script
#
# Start script for prm::camera.capture (front camera)
...
echo "Waiting for front camera topic /head_front_camera/rgb/image_raw..."
TIMEOUT=30
ELAPSED=0
while [ $ELAPSED -lt $TIMEOUT ]; do
    if ros2 topic list 2>/dev/null | grep -q "/head_front_camera/rgb/image_raw"; then
        echo "Front camera topic is available!"
        exit 0
    fi
    sleep 1
    ELAPSED=$((ELAPSED + 1))
...
echo "Warning: Front camera topic not found. Make sure webots is running with robot_launch.py"
exit 1
