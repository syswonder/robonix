...
# Webots's LaserScan publisher has known quirks (reversed
# angle_increment, scan_time/time_increment unset, stamp lags one
# sim step) that wreck downstream SLAM. The lidar primitive owns
# this fix locally — `scan_normalize.py` republishes the raw
# /scanner as a standards-compliant scan, and the primitive
# declares the NORMALISED topic to atlas. This way mapping (and
# any other consumer) only ever sees clean scans; webots-specific
...
docker exec -i \
  ...
  bash -lc "
    ...
    # 1) scan_normalize relay: webots /scanner -> $OUT_TOPIC.
    ...
    # 2) Driver — declares the normalised topic to atlas via env.
