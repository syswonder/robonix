...
def ensure_export_file():
    """Ensure both export files exist for code migration"""
    # Ensure skill directory exists
    os.makedirs(BASE_SKILL_PATH, exist_ok=True)
    
    # Create internal skill/__init__.py if it doesn't exist
    if not os.path.exists(INIT_FILE):
        with open(INIT_FILE, "w") as f:
            f.write("# Auto-generated by @eaios.api\n__all__ = []\n")
    
    # Create external __export__.py if it doesn't exist
...
def update_import_and_all(func_name: str, full_module_path: str):
    """Update both internal skill/__init__.py and external __export__.py files"""
    ensure_export_file()
    import_line = f"from {full_module_path} import {func_name}"

    # Update both files with the same content
...
class eaios:
    ...
    def api(func):
        ...
        def wrapper(**kwargs):
            ...
            if "self_entity" in kwargs:
                ...
                logger.debug(f"{func.__name__}: self_entity already in kwargs, calling func directly")
                return func(**kwargs)
    ...
    def caller(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            skill_module = importlib.import_module("Robonix.skill")  # Must be module path
            logger.debug(f"Skill module id: {id(skill_module)}, file: {skill_module.__file__}")
...
def package_init(config_path: str):
    """
    Initialize package, scan all modules in config_path and register them.
    Updates both internal skill/__init__.py and external __export__.py for code migration.
    """
