    ...
    def api(func):
        ...
        def wrapper(**kwargs):
            ...
            if "self_entity" in kwargs:
                ...
                print(
                    f"[DEBUG] {func.__name__}: self_entity already in kwargs, calling func directly"
                )
                return func(**kwargs)
...
def package_init(config_path: str):
    ...
    for base, entrys in config.items():
        ...
        for entry in entrys:
            ...
            if not os.path.exists(entry_dir):
                print(
                    f"[eaios] Error: The '{entry_dir}' directory was not found at '{base}'"
                )
                continue
    ...
    registry = FunctionRegistry()
    print(
        f"[eaios] Package initialized with {registry.gen_lens()} functions registered."
    )
