...
impl Config {
    ...
    pub fn resolve_source_path(&self, key: SourcePathKey) -> Result<PathBuf> {
        ...
        let abs = match key {
            ...
            // The Cargo workspace lives at the repo root now (was previously
            // under rust/). RustRoot is kept as an alias of Root so downstream
            // consumers asking for "rust" still get something sensible.
            SourcePathKey::RustRoot => root.clone(),
            SourcePathKey::Capabilities => root.join("capabilities"),
            // capabilities/lib is the unified IDL root. Codegen and any
            // downstream IDL search starts from here so msg/srv references in
            // contract TOMLs (e.g. `[io.srv].srv = "demo/srv/Hello"`) have a
