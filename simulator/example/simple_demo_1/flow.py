...
from uapi.runtime.flow import flow, EOS_TYPE_FlowResult, get_runtime
...
def move_a_to_b(a: EntityPath, b: EntityPath) -> EOS_TYPE_FlowResult:
    ...
    if A is None:
        print(f"(move_a_to_b) entity A not found at path: {a}")
        return EOS_TYPE_FlowResult.FAILURE

    if B is None:
        print(f"(move_a_to_b) entity B not found at path: {b}")
        return EOS_TYPE_FlowResult.FAILURE

    b_pos = B.c_space_getpos()
    print(f"(move_a_to_b) entity B position: {b_pos}")

    result = A.c_space_move(x=b_pos["x"], y=b_pos["y"], z=b_pos["z"])
    print(f"(move_a_to_b) move result: {result}")
...
def simple_test_flow() -> EOS_TYPE_FlowResult:
    ...
    print(f"(simple_test_flow) root entity: {graph.get_absolute_path()}")

    children = graph.get_children()
    print(f"(simple_test_flow) root children: {[child.get_absolute_path() for child in children]}")
