...
messages = {
    "initial": (
        "这是纯 Bash 编码代理任务，不涉及机器人硬件。请先执行任务 A1，再执行任务 A2。"  # i18n-ok
        "A1 和 A2 是两个有先后关系、可分别停止的步骤，不要合并成一个 shell command。"  # i18n-ok
        f"任务 A1 的命令是：{a1}。任务 A2 的命令是：{a2}。"  # i18n-ok
    ),
    "greet": (
        "请同时运行一个独立的长期 greet 问候任务，持续运行，不要停止或等待前面的 A 任务。"  # i18n-ok
        f"greet 命令是：{greet_cmd}。"  # i18n-ok
    ),
    "stop_after_a1": (
        "我改变主意了：请在 A1 任务完成之后停止整个 A 计划。"  # i18n-ok
        "持续 greet 任务必须继续运行，不要取消它。"  # i18n-ok
    ),
    "open_area": (
        "请前往开阔区域；在这个 Bash 测试中对应启动一个新的独立任务 C。"  # i18n-ok
        f"任务 C 的命令是：{c_cmd}。持续 greet 仍然保持运行。"  # i18n-ok
    ),
    "cancel_and_return": (
        "取消前往开阔区域对应的任务 C，但不要取消持续 greet。"  # i18n-ok
        "然后返回 315 办公室；在这个 Bash 测试中对应执行任务 D。"  # i18n-ok
        f"任务 D 的命令是：{d_cmd}。"  # i18n-ok
    ),
}
