#!/usr/bin/env python3
"""piper_bridge.py — Piper arm hardware bridge (stdin/stdout JSON protocol).
...
class PiperCollector:
    """Read Piper arm joint motor health via piper_sdk CAN interface."""
...
def main() -> None:
    ...
    print(
        f"[piper_bridge] connected to Piper via {can_port}",
        file=sys.stderr,
        flush=True,
    )
