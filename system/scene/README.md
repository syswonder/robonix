...
| `SCENE_RERUN_HISTORY` | `changes` | `changes` keeps the timeline, so dragging it replays the map filling in as the robot drove. `latest` logs every entity as static: the browser then holds exactly one value per entity and the viewer cannot run out of memory, at the cost of the replay. The occupancy grid is static in both modes. |
