...
# Saved-map previews. Mapping writes `<maps>/<id>/occupancy.png` on every save,
# inside its own container; scene is told where that directory is on the host
# rather than deducing another service's layout. Read-only: the viewer must
# never be what damages the map it is displaying.
if [[ -n "${SCENE_MAP_PREVIEW_DIR:-}" && -d "${SCENE_MAP_PREVIEW_DIR}" ]]; then
    EXTRA_MOUNTS+=(-v "${SCENE_MAP_PREVIEW_DIR}:${SCENE_MAP_PREVIEW_DIR}:ro")
fi
