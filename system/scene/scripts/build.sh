...
BUILD_PROFILE="${ROBONIX_SCENE_BUILD_PROFILE:-full}"
case "$BUILD_PROFILE" in
    full|jetson|lite) ;;
    *)
        echo "[build] error: ROBONIX_SCENE_BUILD_PROFILE must be full, jetson, or lite" >&2
        exit 2
        ;;
esac
if [[ "$BUILD_PROFILE" == "lite" && -z "${ROBONIX_SCENE_IMAGE:-}" ]]; then
    IMG="robonix-scene-lite"
fi
...
PIP_INDEX_BUILD="${ROBONIX_SCENE_PIP_INDEX_URL:-https://pypi.tuna.tsinghua.edu.cn/simple}"
...
ULTRALYTICS_CLIP_WEIGHTS="$WEIGHTS_DIR/ViT-B-32.pt"
if [[ "$BUILD_PROFILE" != "lite" ]]; then
    ...
    fetch_weight \
        "https://github.com/ultralytics/assets/releases/download/v8.1.0/yolov8s-worldv2.pt" \
        "$WEIGHTS_DIR/yolov8s-worldv2.pt"

    fetch_weight \
        "https://github.com/ChaoningZhang/MobileSAM/raw/master/weights/mobile_sam.pt" \
        "$WEIGHTS_DIR/mobile_sam.pt"

    # open_clip is staged once at build time; runtime never downloads models.
    fetch_hf_weight \
        "laion/CLIP-ViT-B-32-laion2B-s34B-b79K" \
        "open_clip_pytorch_model.bin" \
        "$OPEN_CLIP_WEIGHTS"

    # Ultralytics uses OpenAI CLIP only while freezing YOLO-World prompts.
    # Fetch it on the host with the same resumable/cache-aware path as every
    # other large model so Docker builds never perform an opaque download.
    fetch_weight_candidates \
        "$ULTRALYTICS_CLIP_WEIGHTS" \
        "https://openaipublic.azureedge.net/clip/models/40d365715913c9da98579312b702a82c18be219cc2a73407c4526f58eba950af/ViT-B-32.pt"
else
    echo "[build] profile=lite: skipping all perception model downloads"
fi

# ── 2a. jetson-native: host venv, no docker ────────────────────────────────
# The heavy CUDA wheels (torch / torchvision / ultralytics) come from
...
if not str(torch.__version__).startswith("2.8.0"):
    raise RuntimeError(
        "Jetson Orin TensorRT requires the JetPack-compatible torch 2.8.0 wheel; "
        f"found {torch.__version__}"
    )
if not torch.cuda.is_available():
    raise RuntimeError("JetPack torch is installed but CUDA is unavailable")
PY
        echo "[build] error: incompatible Jetson torch/TensorRT host stack" >&2
        echo "        install the exact JetPack 6.2 prerequisites from the Scene README" >&2
        exit 1
    }
    ...
    for req in scene-perception-core; do
        f="docker/requirements/${req}.txt"
        [[ -f "$f" ]] || continue
        echo "[build] pip install --user -r $f"
        "$PY" -m pip install --user -c "$ABI_REQ" -r "$f" \
            --index-url "$PIP_IDX" \
            || echo "[build] warning: some deps in $f failed (perception may degrade)"
    done

    # Orin's supported Torch-TensorRT release is 2.8.  Install the small JPL
    # compiler wheel without dependencies so pip cannot replace the verified
    # JetPack torch or download a second TensorRT runtime.  JetPack supplies
    # TensorRT 10.3 and its Python bindings; ONNX tooling is retained for the
    # one-time fixed-shape YOLO engine export.
    JETSON_PIP_INDEX="${JETSON_PIP_INDEX:-https://pypi.jetson-ai-lab.io/jp6/cu126}"
    TRT_REQ="docker/requirements/scene-jetson-tensorrt.txt"
    "$PY" -m pip install --user -c "$ABI_REQ" -r "$TRT_REQ" \
        --index-url "$JETSON_PIP_INDEX"
    "$PY" -m pip install --user --no-deps \
        "torch-tensorrt==2.8.0+cu126" --index-url "$JETSON_PIP_INDEX"
    "$PY" - <<'PY' || {
import onnx
import onnxruntime
import onnxslim
...
assert str(torch_tensorrt.__version__).startswith("2.8.0")
assert str(tensorrt.__version__).startswith("10.3")
PY
        echo "[build] error: JetPack TensorRT 10.3 / Torch-TensorRT 2.8 runtime is incomplete" >&2
        exit 1
    }
    ...
    # is never placed on the native runtime PYTHONPATH.
    YCD="$PKG/rbnx-build/data/ultralytics"
    UWD="$YCD/weights"
    BAKED_MODELS="$PKG/rbnx-build/data/models"
    BAKED_YOLO="$BAKED_MODELS/yolov8l-world-baked.safetensors"
    BAKED_YOLO_JETSON="$BAKED_MODELS/yolov8s-worldv2-baked.safetensors"
    BAKED_MOBILE_SAM="$BAKED_MODELS/mobile_sam.fp16.safetensors"
    BAKED_OPEN_CLIP="$BAKED_MODELS/open_clip_vit_b32.fp16.safetensors"
    BAKE_DEPS="$PKG/rbnx-build/data/yolo-bake-deps"
    rm -rf "$BAKE_DEPS"
    mkdir -p "$YCD" "$UWD/clip" "$BAKED_MODELS" "$BAKE_DEPS"
    cp -f "$ULTRALYTICS_CLIP_WEIGHTS" "$UWD/clip/ViT-B-32.pt"
...
PY
    "$PY" -m pip install --target "$BAKE_DEPS" --no-deps \
        "clip-anytorch>=2.6" --index-url "$PIP_IDX"
    "$PY" "$PKG/docker/bake_fp16_weights.py" \
        --open-clip-input "$OPEN_CLIP_WEIGHTS" \
        --open-clip-output "$BAKED_OPEN_CLIP" \
        --mobile-sam-input "$WEIGHTS_DIR/mobile_sam.pt" \
        --mobile-sam-output "$BAKED_MOBILE_SAM"
    "$PY" -c "import open_clip; open_clip.create_model_and_transforms('ViT-B-32', pretrained='$BAKED_OPEN_CLIP')"
    PYTHONPATH="$BAKE_DEPS${PYTHONPATH:+:$PYTHONPATH}" \
        YOLO_CONFIG_DIR="$YCD" \
        HF_ENDPOINT="${HF_ENDPOINT:-https://hf-mirror.com}" \
        "$PY" "$PKG/docker/bake_yolo_world.py" \
            --input "$WEIGHTS_DIR/yolov8l-world.pt" \
            --output "$BAKED_YOLO" \
            --architecture yolov8l-world.yaml \
            --classes "$PKG/docker/yolo_world_classes.json"
    PYTHONPATH="$BAKE_DEPS${PYTHONPATH:+:$PYTHONPATH}" \
        YOLO_CONFIG_DIR="$YCD" \
        HF_ENDPOINT="${HF_ENDPOINT:-https://hf-mirror.com}" \
        "$PY" "$PKG/docker/bake_yolo_world.py" \
            --input "$WEIGHTS_DIR/yolov8s-worldv2.pt" \
            --output "$BAKED_YOLO_JETSON" \
            --architecture yolov8s-worldv2.yaml \
            --classes "$PKG/docker/yolo_world_classes.json"
    rm -rf "$BAKE_DEPS" "$UWD/clip"
    [[ -s "$BAKED_YOLO" && -s "${BAKED_YOLO%.safetensors}.classes.json" ]] || {
        echo "[build] error: fixed-vocabulary YOLO-World bake failed" >&2
        exit 1
    }
    [[ -s "$BAKED_YOLO_JETSON" && -s "${BAKED_YOLO_JETSON%.safetensors}.classes.json" ]] || {
        echo "[build] error: fixed-vocabulary Jetson YOLO-World bake failed" >&2
        exit 1
    }
    [[ -s "$BAKED_MOBILE_SAM" && -s "$BAKED_OPEN_CLIP" ]] || {
        echo "[build] error: FP16 perception model bake failed" >&2
        exit 1
    }
...
[[ "$BUILD_PROFILE" == "lite" ]] && SCENE_DOCKERFILE="docker/Dockerfile.lite"
...
DOCKER_BUILD_FLAGS=(
    ...
    --build-arg "SCENE_PIP_INDEX_URL=${PIP_INDEX_BUILD}"
)
...
echo "[build] Python index: ${PIP_INDEX_BUILD} (set ROBONIX_SCENE_PIP_INDEX_URL to override)"
...
if [[ -d "$IDL/src/map" ]]; then
    ...
    # The image dependency set can change without changing generated IDL.
    # In particular, ROS Humble's cached CMake probe may retain the NumPy
    # include path from an older image even after pip replaces the system
    # NumPy package.  Reconfigure this small generated overlay against the
    # image that will actually run it instead of reusing host-side CMake
    # state from a different image.
    rm -rf "$IDL/build" "$IDL/install" "$IDL/log"
