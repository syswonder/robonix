...
from .ingest.perception_tuning import (
...
async def _start_ros_ingest(
    *,
    ...
    # One validated record for every perception knob. Ranges are checked here,
    # once, against the resolved profile: a bad deployment value fails the boot
    # naming the offending `perception.…` path rather than being silently
    # clamped into range inside the detector.
    perception_tuning = PerceptionTuning.from_config(
        perception_cfg,
        profile=perception_profile,
        detect_period_env=os.environ.get("SCENE_DETECT_PERIOD_S", ""),
        yolo_weights_env=(
        ...
        tensor_rt_cache_dir=os.environ.get("SCENE_TENSORRT_CACHE_DIR", ""),
        inference_precision_env=os.environ.get(
            "SCENE_INFERENCE_PRECISION", ""
        ),
        tensor_rt_env=os.environ.get("SCENE_TENSORRT", ""),
    )
    association_cfg = perception_cfg.get("association") or {}
    for legacy_identity_key in DEPRECATED_ASSOCIATION_KEYS:
    ...
    if "rebase_map_corrections" in (perception_cfg.get("geometry") or {}):
        log.warning(
