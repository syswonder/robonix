...
def attach_state(
    *,
    ...
    global _REGISTRY, _HUB, _ROBOT_GEOMETRY, _OBJECT_MUTATIONS
    _REGISTRY = registry
    _HUB = hub
    _ROBOT_GEOMETRY = robot_geometry
    # A coordinator is bound to the registry that created it.  Reattaching
    # Scene state must not leave handlers reading from that previous registry;
    # service startup installs the matching coordinator explicitly below.
    _OBJECT_MUTATIONS = None
