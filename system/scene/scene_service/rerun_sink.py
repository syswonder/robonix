...
def _blueprint_3d():
    ...
    return rrb.Blueprint(
        ...
        # Panels, stated rather than left to `collapse_panels`, which only
        # reaches the first two of these.
        rrb.BlueprintPanel(state="collapsed"),
        rrb.SelectionPanel(state="collapsed"),
        # A live stream has no recording to scrub. The scrubber offers an
        # interaction that does nothing here and takes a strip of the map to
        # say so.
        rrb.TimePanel(state="hidden"),
        # The top bar stays: it carries rerun's menu, which is the route back
        # to everything collapsed above. Not expanding a panel is different
        # from taking it away.
    )
