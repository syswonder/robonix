...
def test_validation_rules():
    ...
    assert ok("region", "kitchen", ROOM_PTS, None) is None
    assert ok("poi", "dock", [[1.0, 2.0]], 1.57) is None
    assert ok("no_go", "x", ROOM_PTS, None)           # unknown kind
    assert ok("region", 7, ROOM_PTS, None)              # non-string name
    assert ok("region", "x" * 200, ROOM_PTS, None)      # name too long
    assert ok("region", "x", ROOM_PTS[:2], None)        # polygon < 3 points
    assert ok("poi", "x", ROOM_PTS[:2], None)         # poi != 1 point
    assert ok("region", "x", [[0.0, float("nan")]] * 3, None)   # NaN point
    assert ok("region", "x", [[0.0], [1.0], [2.0]], None)       # not pairs
    assert ok("region", "x", "not-a-list", None)
    ...
    assert ok("region", "x", ROOM_PTS, 1.57)                    # heading on a region
...
def test_store_roundtrip_across_reopen():
    with tempfile.TemporaryDirectory() as base:
        ...
        region = s1.create(kind="region", name="kitchen", points=ROOM_PTS)
        poi = s1.create(kind="poi", name="dock", points=[[1.0, 2.0]], theta=0.5)
        assert region.annotation_id.startswith("anno.")
        assert region.created_at > 0 and region.updated_at >= region.created_at

        s2 = AnnotationStore(base, map_id="lab", generation=2)
        got = {a.annotation_id: a for a in s2.list()}
        assert got[region.annotation_id].points == ROOM_PTS
        assert got[region.annotation_id].name == "kitchen"
...
def test_room_create_is_idempotent_by_name():
    with tempfile.TemporaryDirectory() as base:
        ...
        a = s.create(kind="region", name="Bathroom", points=ROOM_PTS)
        b = s.create(kind="region", name="  bathroom  ",
...
def test_update_and_delete():
    with tempfile.TemporaryDirectory() as base:
        s = AnnotationStore(base, map_id="lab", generation=1)
        a = s.create(kind="region", name="old", points=ROOM_PTS)
...
def test_map_id_partition_isolation_and_sanitize():
    with tempfile.TemporaryDirectory() as base:
        ...
        s_a.create(kind="region", name="only-in-a", points=ROOM_PTS)
        assert s_b.list() == []
...
def test_delete_map_removes_only_target_partition():
    with tempfile.TemporaryDirectory() as base:
        ...
        target.create(kind="region", name="target region", points=ROOM_PTS)
        other.create(kind="region", name="other region", points=ROOM_PTS)
...
def test_atomic_write_leaves_no_tmp():
    with tempfile.TemporaryDirectory() as base:
        s = AnnotationStore(base, map_id="lab", generation=1)
        s.create(kind="region", name="r", points=ROOM_PTS)
...
def test_generation_mismatch_marks_stale_on_load():
    with tempfile.TemporaryDirectory() as base:
        s1 = AnnotationStore(base, map_id="lab", generation=2)
        a = s1.create(kind="region", name="kitchen", points=ROOM_PTS)
...
def test_unknown_generation_preserves_stored_epoch():
    with tempfile.TemporaryDirectory() as base:
        AnnotationStore(base, map_id="lab", generation=5).create(
            kind="region", name="r", points=ROOM_PTS)
...
def test_redraw_clears_stale_and_mark_all_stale_counts():
    with tempfile.TemporaryDirectory() as base:
        s = AnnotationStore(base, map_id="lab", generation=1)
        a = s.create(kind="region", name="r", points=ROOM_PTS)
...
def test_from_json_drops_unknown_keys():
    ...
    a = Annotation.from_json({
        "annotation_id": "anno.x", "kind": "region", "name": "r",
...
def _call(app, method: str, path: str, body=None):
    ...
    # Split the query off: ASGI carries it separately, and a path with "?" in
    # it matches no route -- which reads as a 404 from the app rather than as
    # a malformed request from the test.
    path, _, query = path.partition("?")
    scope = {
        "type": "http", "http_version": "1.1", "method": method,
        "scheme": "http", "path": path, "raw_path": path.encode(),
        "root_path": "", "query_string": query.encode(), "client": ("test", 0),
        "server": ("test", 80),
...
def test_rest_crud_roundtrip():
    with tempfile.TemporaryDirectory() as base:
        ...
        st, out = _call(app, "GET", "/api/regions")
        assert (st, out) == (200, {"ok": True, "annotations": []})

        st, out = _call(app, "POST", "/api/regions",
                        {"kind": "region", "name": "kitchen-draft", "points": ROOM_PTS})
        assert st == 200 and out["ok"]
        ann_id = out["annotation"]["annotation_id"]

        st, out = _call(app, "PUT", f"/api/regions/{ann_id}",
                        {"name": "kitchen"})
        ...
        st, out = _call(app, "DELETE", f"/api/regions/{ann_id}")
...
def test_map_load_waits_for_new_occupancy_before_rebinding(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as base:
        AnnotationStore(base, map_id="saved_lab", generation=1).create(
            kind="region", name="restored", points=ROOM_PTS,
        )
        ...
        assert [region.name for region in store.list()] == ["restored"]
...
def test_rest_validation_and_errors():
    with tempfile.TemporaryDirectory() as base:
        ...
        st, _ = _call(app, "POST", "/api/regions",
                      {"kind": "region", "name": "x", "points": ROOM_PTS[:2]})
        assert st == 400

        st, out = _call(app, "POST", "/api/regions",
                        {"kind": "region", "name": "x", "points": ROOM_PTS,
                         "theta": 1.57})
        assert st == 400 and "poi" in out["detail"]  # headings are poi-only

        st, _ = _call(app, "PUT", "/api/regions/anno.nope", {"name": "x"})
        assert st == 404
        st, _ = _call(app, "DELETE", "/api/regions/anno.nope")
        assert st == 404

        # A partial update may not sneak in an invalid merged state.
        _, created = _call(app, "POST", "/api/regions",
                           {"kind": "region", "name": "x", "points": ROOM_PTS})
        st, _ = _call(app, "PUT",
                      f"/api/regions/{created['annotation']['annotation_id']}",
                      {"points": [[0.0, 0.0]]})
        assert st == 400

        # Store unavailable → 503 on every verb.
        app_off = _make_web_app(None)
        for method, path in [("GET", "/api/regions"),
                             ("POST", "/api/regions"),
                             ("PUT", "/api/regions/x"),
                             ("DELETE", "/api/regions/x")]:
            st, out = _call(app_off, method, path, {})
            assert st == 503 and not out["ok"]
    print("  [PASS] test_rest_validation_and_errors")


def test_regions_page_served():
    with tempfile.TemporaryDirectory() as base:
        ...
        scope = {"type": "http", "http_version": "1.1", "method": "GET",
                 "scheme": "http", "path": "/regions", "raw_path": b"/regions",
                 # bare=1 asks for the framed document itself: the shell is an
                 # iframe wrapper, so the controls this asserts on are not in it.
                 "root_path": "", "query_string": b"bare=1", "headers": [],
        ...
        body = b"".join(m.get("body", b"") for m in sent
                        if m["type"] == "http.response.body")
        assert status == 200
        assert b"Mark region" in body and b"/api/regions" in body
    print("  [PASS] test_regions_page_served")


def test_state_payload_carries_annotations_and_binding():
    with tempfile.TemporaryDirectory() as base:
        store = AnnotationStore(base, map_id="lab", generation=1)
        store.create(kind="region", name="kitchen", points=ROOM_PTS)
        app = _make_web_app(store)
        if app is None:
            return
